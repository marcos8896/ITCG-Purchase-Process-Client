...
export class BudgetKeyService extends BasicRequestService {
  ...
  public getBudgetKeyWithItsDetails( budgetKeyId ): Observable<any[]> { 
    return this.http.get(`${this.endPoint}/${ budgetKeyId }?filter[include]=budget_key_details&filter[include]=subdirection&filter[include]=program&filter[include]=project`)
      .map(res => res.json() || { })
      .catch(err => JSON.stringify(err));
  }
}
